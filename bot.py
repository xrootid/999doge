		...
		if dogbal > limit:
			...
			while total < target_profit:
				...
				if play['PayOut'] > 0:
					...
					if numws > ws:
						ws +=1
					print(hijau+"[W]",putih+"["+str(ch)+"]"+hijau,"Get From Bet Profit: "+num_format(profit)+putih+" ","Bet : "+num_format(bidshow))
					if base_bet > 0:
						bid = (0 + base_bet) * float(100000000)
					else:
						bid = (0 + base_bet) / float(100000000)
					...
					if numls > ls:
						ls += 1
					print(merah+"[L]",putih+"["+str(ch)+"]"+ merah,"Get From Bet Profit: "+num_format(profit)+putih,"Bet : "+num_format(bidshow))
